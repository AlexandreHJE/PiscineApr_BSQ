...
/*   By: jhu <marvin@42.fr>                         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/26 22:37:29 by jhu               #+#    #+#             */
/*   Updated: 2017/04/26 22:37:30 by jhu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "bsq.h"

int		solve(int **arr, char *str)
{
	int	row;
	int	col;
	...
	{
		arr[row] = (int*)malloc(sizeof(int) * g_settings.cols);
		col = 0;
...
int		bsq(char *filename)
{
	...
	if (g_settings.file < 0)
		return (0);
	parse_settings();
	str = read_buf();
	arr = (int **)malloc(sizeof(int *) * g_settings.rows);
	if (!solve(arr, str))
		return (0);
	print_arr(arr);
	if (filename)
